...
async def get_image(
    ...
    if not md:
        raise HTTPException(status_code=404, detail=f"Image '{name}' not found in database")
    if md.full_path:
...
async def patch_image_metadata(
    ...
    return {"data": (MetadataModel.from_metadata(updated_metadata) if updated_metadata else [])}
